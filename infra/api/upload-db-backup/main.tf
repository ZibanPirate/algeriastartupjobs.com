...
resource "null_resource" "create_tmp_private_key" {
  ...
  depends_on = [data.terraform_remote_state.api]
...
resource "null_resource" "download_db_from_vps" {
  ...
  provisioner "local-exec" {
    command = "scp -i ${path.module}/tmp_private_key -o StrictHostKeyChecking=no -r ${path.module}/../backup-db/sqlite_db_data/${local.stage}/sqlite_db_data ${var.do_droplet_user}@${data.terraform_remote_state.api.outputs.digitalocean_droplet_api_ipv4_address}:${local.api_app_folder}/.."
  }
}
...
resource "ssh_resource" "move_db_file_and_start_api" {
  triggers = {
    always = timestamp()
  }
  ...
  commands = [
    "sudo mkdir -p ${local.api_app_folder}/sqlite_db_data || true",
    "sudo rm -r ${local.api_app_folder}/sqlite_db_data || true",
    "sudo mv ${local.api_app_folder}/../sqlite_db_data ${local.api_app_folder} || true",
